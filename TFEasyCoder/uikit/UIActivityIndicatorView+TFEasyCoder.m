...
+( UIActivityIndicatorView *)easyCoder:(UIActivityIndicatorViewEasyCoderBlock)block{
    return [NSObject tf_execute:[self class] back:^(id ins) {
        if (block) {
            block(( UIActivityIndicatorView *)ins);
        }
    }];
}
...
-(UIActivityIndicatorView  *(^)(long long  activityIndicatorViewStyle))set_activityIndicatorViewStyle{
    __weak typeof(self) weakSelf = self;
    return ^(long long  activityIndicatorViewStyle){
        weakSelf.activityIndicatorViewStyle = activityIndicatorViewStyle;
        return weakSelf;
    };
}
...
-(UIActivityIndicatorView  *(^)(BOOL  hidesWhenStopped))set_hidesWhenStopped{
    ...
    return ^(BOOL  hidesWhenStopped){
        weakSelf.hidesWhenStopped = hidesWhenStopped;
        return weakSelf;
    };
}
