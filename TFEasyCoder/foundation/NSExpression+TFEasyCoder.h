...
typedef void(^NSExpressionEasyCoderBlock) (NSExpression * ins);

@interface NSExpression (TFEasyCoder)

+( NSExpression *)easyCoder:(NSExpressionEasyCoderBlock)block;
-(NSExpression *)easyCoder:(NSExpressionEasyCoderBlock)block;
...
-(NSExpression *(^)(id value,NSString *key))set_ValueKey;
